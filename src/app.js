...
async function briefRun(e) {
  ...
  if (tr) {
    ...
    if (yrs.length >= 2) {
      ...
      if ((a[1] || 0) > 0 || (b[1] || 0) > 0) facts.push('India imports of this line moved from ' + fmtUsd(a[1] || 0) + ' (' + a[0] + ') to ' + fmtUsd(b[1] || 0) + ' (' + b[0] + ').');
    }
  }
  const ss = TRADE_SEASON[c6];
  if (ss) {
...
function openEntry(i) {
  ...
  V.dIdx = i; V.needKey = false; V.busy = false; V.briefError = null; V.copied = false; V.settingsOpen = false; V.plain = null; V.plainBusy = false; V.plainErr = null; V.brief = null; V.briefBusy = false; V.briefErr = null;
  const e = S.db.entries[i];
  const k = e[0] + ':' + e[1];
