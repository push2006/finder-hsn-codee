...
async function tplNarrative(db, idx, apiKey) {
  ...
  const prompt = 'Write the narrative sections for a professional product research report on this exact tariff product.\n' +
    ...
    comtradeFacts(e) +
    'Use Google Search for current facts. Rules: plain simple English, short sentences, readable on a phone. Never invent a number, price, company role, regulation or statistic; rough ranges and qualitative judgements are fine when labelled approximate. Every specific figure you state must name its source and year in the same sentence; where you have no sourced figure, write not available instead of estimating silently. Where product-specific data is unavailable, say so plainly and give clearly labelled general chapter-level context instead. Be specific to THIS product: name real grades, hubs, ports, companies and rules; no filler that could fit any product. Do not use markdown or headings.\n' +
