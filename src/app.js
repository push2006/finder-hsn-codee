    ...
    try { return await PROVIDER_CALL[p](V.apiKey, prompt, opts); }
      ...
      for (const prov of order) {
        if (AI_PROXY_URL) {
          ...
          try { return await PROVIDER_CALL[prov]('', prompt, opts); } catch (e2) { /* next provider */ }
        }
        const alt = builtinKeys(prov);
        if (!alt.length) continue;
        aiSharedKey = true; aiProviderUsed = prov;
        try { return await PROVIDER_CALL[prov](alt, prompt, opts); } catch (e2) { /* next provider */ }
      }
      throw err;
    }
  }
  ...
  if (V.apiKey) {
    ...
    try { return p === 'groq' ? await groqReport(V.apiKey, false) : p === 'gemini' ? await geminiReport(V.apiKey, false) : (aiSharedKey = false, aiProviderUsed = p, await PROVIDER_CALL[p](V.apiKey, prompt, opts)); }
      ...
      for (const prov of order) {
        ...
        const alt = builtinKeys(prov);
        if (!alt.length) continue;
        try { return prov === 'groq' ? await groqReport(alt, true) : prov === 'gemini' ? await geminiReport(alt, true) : (aiSharedKey = true, aiProviderUsed = prov, await PROVIDER_CALL[prov](alt, prompt, opts)); } catch (e2) { /* next provider */ }
      }
      throw err;
    }
  }
