...
function briefHtml(e) {
  ...
  return '<div class="detail-sec no-print" id="brief-card"><h3>Trade brief - this code in a paragraph</h3><div id="brief-slot">' + inner + '</div>' +
...
function docExplHtml(e) {
  ...
  return '<div class="detail-sec no-print" id="doc-card"><h3>Documents explained - plain words</h3><div id="doc-slot">' + inner + '</div>' +
...
async function docExplRun(e) {
  ...
  try {
    const t = await aiPickText('Explain the export paperwork for this Indian product to a first-time small exporter.\n' + facts.join('\n') + '\nRules: use ONLY the facts above - never invent documents, certificates, authorities, numbers or rules; keep each explanation tied to the exact document named; one short sentence per document or certificate: what it is and why it is needed; simple words, no jargon; plain text only - no markdown, no asterisks, no bullet symbols; at most 12 sentences; end with exactly: Your customs broker confirms the live list for each shipment.', { temperature: 0.3, maxTokens: 900 });
...
function cmpVerdictHtml(a, b) {
  ...
  return '<div class="detail-sec no-print" id="cmpv-card" style="margin-top:14px"><h3>Compare verdict - which code for what</h3><div id="cmpv-slot">' + inner + '</div>' +
...
function shipSanCardHtml() {
  ...
  return '<div class="detail-sec sanctions-card no-print"><h3>Ship sanction check - vessel name or IMO</h3>' +
    ...
    '</div>' +
    '<p class="muted src-line">Source: <a href="https://sanctionslistservice.ofac.treas.gov/api/download/sdn.xml" target="_blank" rel="noreferrer">OFAC SDN</a> (CSV dated 22 Sep 2026), ' + SHIP_SANC.length.toLocaleString('en-US') + ' sanctioned vessels. Not legal advice.</p>' +
    '</div>';
}
