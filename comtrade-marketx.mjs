// Market data bake: top exporting countries for EVERY 6-digit HS code.
// Strategy: pull each reporting country's full AG6 world-export table (one bulk
...
// State lives in state/comtrade-marketx.json so partial progress survives reruns;
// a 429 stops the run cleanly and the next run resumes.
import fs from 'fs';

const KEY = process.env.COMTRADE_KEY || '';
const DRY = process.env.RENDER_DRY === '1';
const STATE = 'state/comtrade-marketx.json';
const OUT = 'src/marketexp.ts';
...
async function pullBatch(ids, year) {
  const url = `https://comtradeapi.un.org/data/v1/get/C/A/HS?reporterCode=${ids.join('%2C')}&period=${year}&partnerCode=0&partner2Code=0&flowCode=X&cmdCode=AG6&customsCode=C00&motCode=0`;
  const d = await fetchJson(url);
  return d.data || [];
}

export async function bakeMarketX() {
...
function writeOutputs(st, reporters, year) {
  // Rank per code: top 8 exporting countries by value.
  const top = {};
  ...
  let out = `// Top exporting countries by 6-digit HS code, calendar year ${year}.\n` +
...
if (process.argv[1] && process.argv[1].endsWith('comtrade-marketx.mjs')) {
  bakeMarketX().then((r) => console.log('[ok] comtrade-market', JSON.stringify(r))).catch((e) => { console.error('[fail] comtrade-market', e.message); process.exit(1); });
}
